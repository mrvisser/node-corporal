...
var CorporalHistory = require('./history');
 ...
 */
function _getCommand(session, initialStr, callback, _rl, _str) {
    // Create the keypress handler method that will help reading from session command
    // history with up and down commands
    var _handleKeypress = null;
    if (!_rl) {
        _rl = _createReadline(session);

        _handleKeypress = _createHandleKeypress(session, _rl, _str, callback);

        // Handle CTRL+C to clear the prompt rather than kill the process
        _rl.once('SIGINT', function() {

            // Toggle back to the end of the command history
            session.history().reset();

            // We're going to get the next command while tossing away the
            // current input (_str) and the current _rl session
            _rl.close();
            console.log('');
            process.stdin.removeListener('keypress', _handleKeypress);
            return _getCommand(session, '', callback);
        });

        // Replace any current keypress listener with a new one
    ...
    _rl.question(ps, function(str) {
        ...
        if (result.closed) {
            // Push this full command into the command history
            if (!_.isEmpty(result.str.trim())) {
                session.history().push(result.str);
            }
            session.history().reset();

            // The input closed off (i.e., user didn't pull the
            // command to the next line). So we can close the
            // readline session and return the parsed result
            _rl.close();
            process.stdin.removeListener('keypress', _handleKeypress);
    ...
    // Place the initial string in the command line
    _rl.write(initialStr);
}

/*!
 * Creates a function that will iterate through the command history when
 * up and down arrows are used
 */
function _createHandleKeypress(session, rl, currentStr, callback) {
    var _handleKeypress = function(ch, key) {
        if (!key || currentStr) {
            // Ignore ups and downs while in multi-line mode
            return;
        }

        // Only listen to ups and downs
        if ((key.name === 'up' || key.name === 'down') && !key.ctrl && !key.shift && !key.meta) {
            // Determine if we are making a useful move in the history
            var replace = null;
            if (key.name === 'up') {
                replace = session.history().prev();
            } else if (key.name === 'down') {
                replace = session.history().next();
            }

            // If we actually moved to something in the history, replace the current line with it
            if (_.isString(replace)) {
                rl.close();
                process.stdin.removeListener('keypress', _handleKeypress);
                return _getCommand(session, replace, callback);
            }
        }
    };
 ...
 * color prompts
 */
function _createReadline(session) {
...
function _parseArgs(str) {
    ...
    if (state.ESCAPE_NEXT) {
        ...
        // command
        args.push(state.CURR_ARG);
        return {'str': str, 'args': args, 'closed': true};
    }
}
